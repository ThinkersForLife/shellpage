...
function load_puzzle(){"use strict";
	...
	if (image_slider.getPos()!==0) {
		...
		Commented by Jay Shah on 7th Sep., 2015 17:41:41
*/
			g_imagepath =	$("#image1-1").attr("src");
	} else {
		/*g_imagepath = g_ownimage;
		
		Commented by Jay Shah on 7th Sep., 2015 16:13:59
		...
		if ((piece.getDraggable()||piece.getParent().getDraggable()) && g_ready) {
			if (g_elastic && piece.getParent().attrs.name === "g_layer") {
				...
				trans = piece.getParent().transitionTo({
					duration: 0.5,
					easing: 'elastic-ease-out',
					//shadowOffset: {x: 5,y: 5},g\E4g\E4g\E4
					scale: {x: 1,y: 1},
					callback: function() {teil_zieltest (piece);}
				});
...
function teil_setzen (piece) {"use strict";
	...
	document.getElementById("container").width = 0;
	document.getElementById("container").height = 0;
...
function back() {"use strict";
	...
	//$.mobile.changePage('#title', {transition: 'slide'});
	content_formatting();
...
/*			if (localStorage.getItem('s_theme') === null)       {$('#select_theme').val("Europe");} else {$('#select_theme').val(localStorage.getItem('s_theme'));}
		}
		Commented by Jay Shah on 7th Sep., 2015 16:13:35	
*/

/*
		ADD A FOLDER NAME HERE WHERE IMAGE EXISTS
		e.g. Replace "asia" with "new_folder_name"
*/
$('#select_theme').val("asia");

}
		// Example usage - http://homepage.hispeed.ch/grrds_games/Puzzle/?theme=europe

		/*var url_param = url_query('theme');
		if( url_param ) {
			$('#select_theme').val(url_param);
		}
		Commented by Jay Shah on 7th Sep., 2015 16:01:40
		*/
	...
	for(var s = 1; s < 5; s++) { 
		for(var n = 1; n < 4; n++) {
			...
			/*$("#image"+s+"-"+n).attr("src","Images/"+$('#select_theme').val()+"/image-set-"+s+"/sujet"+g_portrait+n+"s.jpg"); 
				...
				Commented by Jay Shah on 7th Sep., 2015 17:49:05
