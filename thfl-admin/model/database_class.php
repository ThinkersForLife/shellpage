...
//											$query = $this->pdo->prepare('SELECT companyName,email,name,totalScr FROM '.RESULT_TABLE.' WHERE totalScr <> 0 GROUP BY email ORDER BY totalScr DESC');
											$query = $this->pdo->prepare('SELECT edate,companyName,email,location,name,SUM(totalScr) as totalScr,MAX(rawScr) as rawScr FROM '.RESULT_TABLE.' WHERE totalScr > 0 AND email!="" GROUP BY email ORDER BY totalScr DESC');
...
//								echo $limit;
							...
							if($limit==-1){
										$query = $this->pdo->prepare('SELECT edate,name,email,location,minScr,maxScr,totalGameTime,companyName,SUM(totalScr) as totalScr FROM '.RESULT_TABLE.' WHERE email!="" GROUP BY email ORDER BY totalScr DESC');
										$query_old = $this->pdo->prepare('SELECT edate,name,email,location,minScr,maxScr,totalGameTime,companyName,SUM(totalScr) as totalScr FROM '.RESULT_TABLE.' WHERE totalScr > 0 GROUP BY email ORDER BY totalScr DESC');
										//$query = $this->pdo->prepare('SELECT * FROM '.RESULT_TABLE.' ORDER BY totalScr DESC');
							}
							else{
...
//										$query = $this->pdo->prepare('SELECT companyName,email,name,SUM(totalScr) as totalScr,MAX(rawScr) as rawScr FROM '.RESULT_TABLE.' WHERE totalScr <> 0 GROUP BY email ORDER BY totalScr DESC LIMIT 10');
										$query2 = $this->pdo->prepare('SELECT edate,companyName,name,max(totalScr) as rawScr,email FROM '.RESULT_TABLE.' WHERE totalScr <> 0 GROUP BY email ORDER BY totalScr DESC LIMIT 10');												
							}
